...
BIOMETRIC_MODEL_PATH = os.path.join(BASE_DIR, "03_models", "uidai_biometric_model.pkl")
DEMOGRAPHIC_MODEL_PATH = os.path.join(BASE_DIR, "03_models", "uidai_demographic_model.pkl")
DEMOGRAPHIC_MODEL_INFO_PATH = os.path.join(BASE_DIR, "03_models", "uidai_demographic_model_info.pkl")
...
DEMOGRAPHIC_DATA_PATH = os.path.join(BASE_DIR, "01_data", "processed", "clean_demographic_data.csv")
...
from schemas import (
    ...
    BiometricPredictionRequest, BiometricPredictionResponse
)
...
class ModelService:
    def __init__(self):
        ...
        self.demographic_model_info = None
        self.data = None
    ...
    def load_resources(self):
        ...
        # 3. Load Biometric Model
        if os.path.exists(BIOMETRIC_MODEL_PATH):
            try:
                self.biometric_model = joblib.load(BIOMETRIC_MODEL_PATH)
                print(f"[INFO] Biometric Model loaded from {BIOMETRIC_MODEL_PATH}")
            except Exception as e:
                print(f"[ERROR] Failed to load biometric model: {e}")
        else:
            print(f"[WARN] Biometric Model not found at {BIOMETRIC_MODEL_PATH}")

        # 4. Load Demographic Model
        if os.path.exists(DEMOGRAPHIC_MODEL_PATH):
            try:
                self.demographic_model = joblib.load(DEMOGRAPHIC_MODEL_PATH)
                print(f"[INFO] Demographic Model loaded from {DEMOGRAPHIC_MODEL_PATH}")
                
                # Load model info (feature columns, etc.)
                if os.path.exists(DEMOGRAPHIC_MODEL_INFO_PATH):
                    self.demographic_model_info = joblib.load(DEMOGRAPHIC_MODEL_INFO_PATH)
                    print(f"[INFO] Demographic Model Info loaded")
            except Exception as e:
                print(f"[ERROR] Failed to load demographic model: {e}")
        else:
            print(f"[WARN] Demographic Model not found at {DEMOGRAPHIC_MODEL_PATH}")

        # 5. Load Biometric History Data (Primary data for dashboard and predictions)
        try:
            if os.path.exists(DATA_PATH):
                print(f"[INFO] Loading biometric data from {DATA_PATH}...")
                ...
                print(f"[INFO] Biometric Data Loaded: {len(self.data)} records.")
            else:
                print(f"[ERROR] CRITICAL: Biometric data file missing.")
        except Exception as e:
            print(f"[ERROR] Failed to load biometric data: {e}")
            
        # 5. Load Enrollment Data (For enrollment predictions)
        try:
            if os.path.exists(ENROLLMENT_DATA_PATH):
                print(f"[INFO] Loading enrollment data from {ENROLLMENT_DATA_PATH}...")
                self.enrollment_data = pd.read_csv(ENROLLMENT_DATA_PATH, low_memory=False)
                if 'date' in self.enrollment_data.columns:
                    self.enrollment_data['date'] = pd.to_datetime(self.enrollment_data['date'], errors='coerce')
                print(f"[INFO] Enrollment Data Loaded: {len(self.enrollment_data)} records.")
        except Exception as e:
            print(f"[ERROR] Failed to load enrollment data: {e}")
            
        # 6. Load Demographic Data (For demographic predictions)
        try:
            if os.path.exists(DEMOGRAPHIC_DATA_PATH):
                print(f"[INFO] Loading demographic data from {DEMOGRAPHIC_DATA_PATH}...")
                self.demographic_data = pd.read_csv(DEMOGRAPHIC_DATA_PATH, low_memory=False)
                if 'date' in self.demographic_data.columns:
                    self.demographic_data['date'] = pd.to_datetime(self.demographic_data['date'], errors='coerce')
                print(f"[INFO] Demographic Data Loaded: {len(self.demographic_data)} records.")
        except Exception as e:
    ...
    def _get_historical_context(self, state: str, district: str, model_type: str = 'demand'):
        """Retrieves lag features and history from loaded dataset for future trend prediction."""
        ...
        # These lag features help the ML model understand recent trends to predict future
        
        lags = {}
        if model_type == 'demand':
            # Use the last 3 months of actual data to predict future trends
            if len(district_data) >= 3:
                last_3_vals = district_data.tail(3)[['bio_age_5_17', 'bio_age_17_']].sum(axis=1).values
                lags = {
                    'lag_1m_bio': int(last_3_vals[-1]),  # Most recent month
                    'lag_2m_bio': int(last_3_vals[-2]),  # 2 months ago
                    'lag_3m_bio': int(last_3_vals[-3]),  # 3 months ago
                    'rolling_std_3m': float(last_3_vals.std())  # Volatility measure
                }
            elif len(district_data) >= 1:
                recent_total = district_data.iloc[-1]['bio_age_5_17'] + district_data.iloc[-1]['bio_age_17_']
                lags = {
                    'lag_1m_bio': int(recent_total),
                    'lag_2m_bio': int(district_data.iloc[-2]['bio_age_5_17'] + district_data.iloc[-2]['bio_age_17_']) if len(district_data) > 1 else 0,
                    'lag_3m_bio': 0,
                    'rolling_std_3m': 0.0
                }
            else:
                lags = {'lag_1m_bio': 0, 'lag_2m_bio': 0, 'lag_3m_bio': 0, 'rolling_std_3m': 0.0}
        else: # Enrollment
            if len(district_data) >= 3:
                lags = {
                    'lag_1m_adult': int(district_data.iloc[-1]['bio_age_17_']),
                    'lag_2m_adult': int(district_data.iloc[-2]['bio_age_17_']),
                    'lag_3m_adult': int(district_data.iloc[-3]['bio_age_17_']),
                    'rolling_mean_3m': float(district_data.tail(3)['bio_age_17_'].mean())
                }
            else:
                lags = {
                    'lag_1m_adult': int(district_data.iloc[-1]['bio_age_17_']) if len(district_data) > 0 else 0,
                    'lag_2m_adult': int(district_data.iloc[-2]['bio_age_17_']) if len(district_data) > 1 else 0,
                    'lag_3m_adult': 0,
                    'rolling_mean_3m': 0.0
                }
        
        # History for UI - show last 6 months of actual data
        history_points = []
    ...
    def predict_single(self, request: PredictionRequest) -> PredictionResponse:
        """
        Predict FUTURE trends based on historical patterns and current inputs.
        
        How Future Prediction Works:
        1. Extracts last 3 months of historical data (lag features) for the district
        2. Uses these patterns + user's future month/year input
        3. ML model (RandomForestRegressor) trained on 1.86M historical records
        4. Predicts future demand by learning from:
           - Seasonal patterns (month of year)
           - Recent trends (lag_1m, lag_2m, lag_3m)
           - District characteristics
           - Age group distributions
        5. Returns predicted value for the FUTURE month specified by user
        """
        ...
        # 1. Fetch Historical Context (Last 3 months actual data)
        # These lag features allow the model to understand recent trends and predict future
    ...
    def predict_biometric(self, request: BiometricPredictionRequest) -> BiometricPredictionResponse:
        if not self.biometric_model:
            # Fallback mock if model fails to load in dev
            print("[WARN] Using Mock Biometric Prediction")
            return BiometricPredictionResponse(
                predicted_bio_total=request.age_0_5 * 0.5 + request.age_5_17 * 0.8 + 100,
                confidence_score=0.75
            )

        # Calculate bio_total for feature engineering
        bio_total = request.age_0_5 + request.age_5_17 + request.age_18_greater
        
        # Create base features
        input_data = pd.DataFrame([{
            'age_0_5': request.age_0_5,
            'age_5_17': request.age_5_17,
            'age_18_greater': request.age_18_greater,
            'month': request.month,
            'day_of_week': request.day_of_week,
            # Additional features required by the model
            'quarter': (request.month - 1) // 3 + 1,  # Quarter (1-4)
            'is_weekend': 1 if request.day_of_week >= 5 else 0,  # Weekend indicator
            'day_of_month': 15,  # Default to mid-month
            # Lag features (use bio_total as estimate for historical values)
            'bio_total_lag_1': bio_total * 0.95,  # Approximate lag values
            'bio_total_lag_7': bio_total * 0.93,
            'bio_total_lag_30': bio_total * 0.90,
            # Rolling features
            'bio_total_rolling_mean_7': bio_total * 0.94,
            'bio_total_rolling_std_7': bio_total * 0.05,
            # Ratio features
            'ratio_5_17': request.age_5_17 / bio_total if bio_total > 0 else 0,
            'ratio_17_plus': request.age_18_greater / bio_total if bio_total > 0 else 0,
            # Interaction feature
            'age_month_interaction': request.age_5_17 * request.month
        }])

        prediction = self.biometric_model.predict(input_data)[0]
        
        return BiometricPredictionResponse(
            predicted_bio_total=float(prediction),
            confidence_score=0.85 
        )

    def predict_demographic(self, state: str, district: str, year: int, month: int):
        """
        Predict demographic demand using the trained demographic model
        """
        if not self.demographic_model or self.demographic_data is None:
            print("[WARN] Demographic model or data not loaded")
            return {
                "prediction": 25,
                "confidence": 70,
                "trend": 0.0,
        ...
        # Get state and district encodings
        state_df = self.demographic_data[self.demographic_data['state'] == state]
        if state_df.empty:
            print(f"[WARN] State '{state}' not found in demographic data")
            return {
                "prediction": 25,
                "confidence": 70,
                "trend": 0.0,
        ...
        if district_df.empty:
            print(f"[WARN] District '{district}' not found in state '{state}'")
            return {
                "prediction": 25,
                "confidence": 70,
                "trend": 0.0,
        ...
        # Get encoded values
        state_encoded = district_df['state_encoded'].iloc[0]
        district_encoded = district_df['district_encoded'].iloc[0]
        
        # Get historical monthly aggregates for this location
        monthly_data = self.demographic_data[
            (self.demographic_data['state_encoded'] == state_encoded) &
            (self.demographic_data['district_encoded'] == district_encoded)
        ].copy()
        ...
        monthly_agg = monthly_data.groupby(['year', 'month']).agg({
            'demo_age_5_17': ['mean', 'sum', 'std'],
            'demo_age_17_': ['mean', 'sum', 'std'],
            'total_demographic': ['mean', 'count']
        }).reset_index()
        ...
        # Create lag features
        for lag in [1, 2, 3]:
            monthly_agg[f'target_lag_{lag}'] = monthly_agg['total_demographic_mean'].shift(lag)
            monthly_agg[f'age_5_17_lag_{lag}'] = monthly_agg['demo_age_5_17_mean'].shift(lag)
            monthly_agg[f'age_17_lag_{lag}'] = monthly_agg['demo_age_17__mean'].shift(lag)
        
        # Get last valid row for lag features
        last_row = monthly_agg.dropna().tail(1)
        
        if last_row.empty:
            print(f"[WARN] No historical data for {district}, {state}")
            return {
                "prediction": 25,
                "confidence": 70,
                "trend": 0.0,
        ...
        feature_cols = self.demographic_model_info['feature_columns']
        ...
        input_data = pd.DataFrame([{
            ...
            'year': year,
            'month': month,
            'demo_age_5_17_mean': last_row['demo_age_5_17_mean'].iloc[0],
            'demo_age_5_17_sum': last_row['demo_age_5_17_sum'].iloc[0],
            'demo_age_5_17_std': last_row['demo_age_5_17_std'].iloc[0],
            'demo_age_17__mean': last_row['demo_age_17__mean'].iloc[0],
            'demo_age_17__sum': last_row['demo_age_17__sum'].iloc[0],
            'demo_age_17__std': last_row['demo_age_17__std'].iloc[0],
            'target_count': last_row['total_demographic_count'].iloc[0],
            'target_lag_1': last_row['target_lag_1'].iloc[0],
            'target_lag_2': last_row['target_lag_2'].iloc[0],
            'target_lag_3': last_row['target_lag_3'].iloc[0],
            'age_5_17_lag_1': last_row['age_5_17_lag_1'].iloc[0],
            'age_5_17_lag_2': last_row['age_5_17_lag_2'].iloc[0],
            'age_5_17_lag_3': last_row['age_5_17_lag_3'].iloc[0],
            'age_17_lag_1': last_row['age_17_lag_1'].iloc[0],
            'age_17_lag_2': last_row['age_17_lag_2'].iloc[0],
            'age_17_lag_3': last_row['age_17_lag_3'].iloc[0]
        }])
        
        # Make prediction
        prediction = self.demographic_model.predict(input_data[feature_cols])[0]
        
        # Calculate trend
        prev_value = last_row['total_demographic_mean'].iloc[0]
        trend = ((prediction - prev_value) / prev_value * 100) if prev_value > 0 else 0.0
        
        return {
            "prediction": int(round(prediction)),
            "confidence": 85,
            "trend": float(round(trend, 1)),
            "state": state,
            "district": district
        }
