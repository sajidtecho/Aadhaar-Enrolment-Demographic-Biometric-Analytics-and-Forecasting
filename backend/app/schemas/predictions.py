...
class BiometricPredictionRequest(BaseModel):
    age_0_5: float = Field(..., description='Enrollment count for ages 0-5')
    age_5_17: float = Field(..., description='Enrollment count for ages 5-17')
    age_18_greater: float = Field(..., description='Enrollment count for ages 18+')
    month: int = Field(..., ge=1, le=12)
    day_of_week: int = Field(0, ge=0, le=6, description='0=Monday, 6=Sunday')

class BiometricPredictionResponse(BaseModel):
    predicted_bio_total: float
    confidence_score: float = 0.0 # Placeholder
